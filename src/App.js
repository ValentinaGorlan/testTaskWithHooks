  ...
  return (
    <div className={dark ? "app_wrapper dark" : "app_wrapper"}>
      <div className="menu_wrapper">
        ...
        <div className="btn_container">
          <button className="theme_btn" onClick={openSettings}>
          ...
          <div style={{ display: display }} className="theme_modal">
            ...
            <label
              for="highload1"
              data-onlabel="Ativada"
              data-offlabel="Desativado"
              class="lb1"
            ></label>
          </div>
        </div>
      ...
      {index > -1 ? (
        <MySlider data={Data()} ind={index} />
      ) : (
        <Album data={() => searchImg(Data(), term)} findIndex={findIndex} />
      )}
    </div>
  );
}
